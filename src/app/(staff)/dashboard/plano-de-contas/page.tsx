...
const initialRows: AccountRow[] = ((parametrizationCache as {
  chartAccounts?: CachedChartAccount[];
}).chartAccounts ?? []).map((account) => ({
  id: account.id,
  code: account.code,
  reducedCode: account.reduced_code ?? "-",
  level: account.level,
  type: account.level >= 5 ? "A" : "T",
  description: account.name,
  alias: account.report_category ?? "-",
  report: account.report_type ?? "Balanco Patrimonial",
}));
...
function parseImportedAccounts(rows: ImportedRow[]): AccountRow[] {
  return rows
    .map((row, index) => {
      const code = String(
        getRowValue(row, ["codigo", "código", "code", "cod"])
      ).trim();
...
export default function PlanoDeContasPage() {
  ...
  const [importedCount, setImportedCount] = useState(initialRows.length);
        ...
        <div className="mt-6 overflow-hidden rounded-[1.6rem] border border-white/10 bg-[#0b1424] shadow-[0_18px_50px_rgba(0,0,0,0.22)]">
          <div className="flex items-center justify-between gap-3 border-b border-white/10 bg-[#091223] px-5 py-4">
            ...
            <p className="text-sm text-slate-500">
              {filteredRows.length} contas
              {importedCount !== filteredRows.length ? ` de ${importedCount}` : ""}
            </p>
          </div>
